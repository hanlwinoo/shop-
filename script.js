...
function initializeAdminPage() {
    ...
    async function addShipping(event) {
        ...
        const shipmentData = {
            ...
            origin,
            destination,
            status: currentLang === 'zh' ? initialStep.zh : initialStep.en,
            createdAt: timestamp,
            updatedAt: timestamp,
            trackingHistory: [{
                step: 1,
                date: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
                description: currentLang === 'zh' ? initialStep.zh : initialStep.en,
                location: origin
            }]
        };
    ...
    async function loadShipments() {
        try {
            ...
            list.forEach((shipment) => {
                ...
                if (statusUpper.includes('TRANSIT') || statusUpper.includes('HUB')) {
                ...
                tr.innerHTML = `
                    ...
                    <td class="col-status">
                        <span class="status-pill ${statusClass}">${(shipment.status || 'PENDING').toUpperCase()}</span>
                    </td>
    ...
    if (updateStatusForm) {
        updateStatusForm.addEventListener('submit', async (e) => {
            ...
            const stepText = currentLang === 'zh' ? selectedStepObj.zh : selectedStepObj.en;
